...
class SbbPrizeDay extends Sbb
{
    ...
    public function dataCount($results)
    {
        foreach ($results as $key => $result) {
            ...
            $result['总玩分数'] = $result['总玩分数'] + $result['加买总玩分数'];
            $result['总赢分数'] = $result['总赢分数'] + $result['加买总赢分数'];
            $result['总玩局数'] = $result['总玩局数'];
            $result['总赢局数'] = $result['总赢局数'] + $result['加买总赢局数'];

            $results[$key]['盈利']    = round(($result['总玩分数'] - $result['总赢分数']) / 100, 2);
            ...
            $results[$key]['加买几率']  = $result['加买总玩分数'] == 0 ? "0%" : round($result['加买总赢分数'] / $result['加买总玩分数'] * 100, 2) . "%";
            $results[$key]['加买率']   = $result['总玩局数'] == 0 ? "0%" : round($result['加买总玩局数'] / $result['总玩局数'] * 100, 2) . "%";
            $results[$key]['加买中奖率'] = $result['加买总玩局数'] == 0 ? "0%" : round($result['加买总赢局数'] / $result['加买总玩局数'] * 100, 2) . "%";
        }
        return $results;
    }
    ...
    public function LocusToPrizeMerge($locus, $compareObjs, &$result)
    {
        ...
        Tool::issetInitValue($result['add_buy_play_number'], 0);//加买总玩局数
        Tool::issetInitValue($result['add_buy_win_number'], 0);//加买总赢局数
        Tool::issetInitValue($result['add_buy_play_score'], 0);//加买总玩分数
        Tool::issetInitValue($result['add_buy_win_score'], 0);//加买总赢分数
        Tool::issetInitValue($result['gold_card'], 0);//黄金牌
        Tool::issetInitValue($result['platinum_card'], 0);//白金牌
        ...
        if ($locus['prize_award_type'] != "" && $locus['prize_award_num']) {
            ...
            //这里只记录不是红包来袭的奖
            $prizeIdArr = explode(",", $locus['prize_id']);
            foreach ($prizeIdArr as $prizeId) {
                Tool::issetInitValue($result[$prizeId], 0);
                $result[$prizeId] += 1;
            }
        }
        ...
        $result['add_buy_play_number'] += $locus['add_buy_play_number'];
        $result['add_buy_play_score']  += $locus['add_buy_play_score'];
        $result['add_buy_win_number']  += $locus['add_buy_win_number'];
        $result['add_buy_win_score']   += $locus['add_buy_win_score'];

        if( $locus['plus_cards_type'] == 2){
            $result['gold_card'] += 1;
        }else if($locus['plus_cards_type'] == 3){
            $result['platinum_card'] += 1;
        }
    ...
    public function LocusToPrizeDay($locus, $param)
    {
        ...
        foreach ($locus as $key => $val) {
            ...
            $compareJson['add_buy_play_number'] = $val['add_buy_play_number'];//加买总玩局数
            $compareJson['add_buy_win_number']  = $val['add_buy_win_number'];//加买总赢局数
            $compareJson['add_buy_play_score']  = $val['add_buy_play_score'];//加买总玩分数
            $compareJson['add_buy_win_score']   = $val['add_buy_win_score'];//加买总赢分数
            $compareJson['gold_card']           = $val['gold_card'];//黄金牌
            $compareJson['platinum_card']       = $val['platinum_card'];//白金牌
            ...
            unset($val['account_id'], $val['machine_auto_id'], $val['room_index']
                ...
                , $val['add_buy_play_number'], $val['add_buy_win_number'], $val['add_buy_play_score'], $val['add_buy_win_score']
                , $val['gold_card'], $val['platinum_card']
            );
    ...
    public function prizeObjToChinese(&$result, $obj, $newPrizeList)
    {
        ...
        Tool::issetInitValue($result['加买总玩局数'], 0);
        Tool::issetInitValue($result['加买总赢局数'], 0);
        Tool::issetInitValue($result['加买总玩分数'], 0);
        Tool::issetInitValue($result['加买总赢分数'], 0);
        Tool::issetInitValue($result['黄金牌'], 0);
        Tool::issetInitValue($result['白金牌'], 0);

        $result['加买总玩局数'] += $compareJsonArr['add_buy_play_number'];
        $result['加买总赢局数'] += $compareJsonArr['add_buy_win_number'];
        $result['加买总玩分数'] += $compareJsonArr['add_buy_play_score'];
        $result['加买总赢分数'] += $compareJsonArr['add_buy_win_score'];
        $result['黄金牌']    += $compareJsonArr['gold_card'];
        $result['白金牌']    += $compareJsonArr['platinum_card'];
