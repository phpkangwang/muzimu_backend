...
class ByuLocus extends Byu
{
    ...
    public function LocusPage($params)
    {
        ...
        $newRoomList = $params['newMachineList'];
        ...
        $where = " loc.account_id = '{$params['accountId']}' ";
        if ($lastId != "") {
            $where .= " and loc.id < '{$params['lastId']}'";
        }

        if ($params['isKill'] != "") {
            $where .= " and loc.is_kill = '{$params['isKill']}'";
        }
        ...
        if ($params['prizeId'] != "") {
            $where .= " and loc.fish_id = '{$params['prizeId']}' ";
        }
        ...
        foreach ($data as $key => $val) {
            ...
            $data[$key]['prizeName']    = isset($newPrizeList[$val['fish_id']]['desc']) ? $newPrizeList[$val['fish_id']]['desc'] : "";
            $data[$key]['roomName']    = isset($newRoomList[$val['room_index']]['name']) ? $newRoomList[$val['room_index']]['name'] : "";
        }
        return $data;
    }
