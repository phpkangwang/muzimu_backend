...
class MxjPrizeDay extends Mxj
{
    ...
    public function dataCount($results)
    {
        foreach ($results as $key => $result) {
            ...
            if (Yii::$app->params['payType'] == 2) {
                //使用的是分数
                $result['总送分'] = $result['总送钻'];
            } else {
                //使用的是钻石
                $result['总送分'] = $result['总送钻'] * 100;
            }

            $results[$key]['总送分']     = $result['总送钻'];
            $results[$key]['送分后游戏机率'] = $result['总玩分数'] == 0 ? "0%" : round(($result['总赢分数'] + $result['总彩金'] + $result['总送分']) / $result['总玩分数'] * 100, 2) . "%";
            $results[$key]['送钻后游戏机率'] = $results[$key]['送分后游戏机率'];
            $results[$key]['盈利']      = round(($result['总玩分数'] - $result['总赢分数'] - $result['总彩金'] - $result['总送分']) / 100, 2);
            $results[$key]['游戏机率']    = $result['总玩分数'] == 0 ? "0%" : round(($result['总赢分数'] + $result['总彩金']) / $result['总玩分数'] * 100, 2) . "%";
    ...
    public function PlayerRecordNever($accountId)
    {
        ...
        $resultSum = $this->dataCount($resultSum);
        foreach ($resultSum as $key => $val) {
            $compareJson              = json_decode($val['compare_json'], true);
            $resultSum[$key]['两倍明星奖'] = isset($compareJson['star_two']) ? $compareJson['star_two'] : 0;
            $resultSum[$key]['三倍明星奖'] = isset($compareJson['star_three']) ? $compareJson['star_three'] : 0;
            $resultSum[$key]['四倍明星奖'] = isset($compareJson['star_four']) ? $compareJson['star_four'] : 0;
            $resultSum[$key]['单樱桃']   = isset($compareJson['aline_one_cherry']) ? $compareJson['aline_one_cherry'] : 0;
            $resultSum[$key]['双樱桃']   = isset($compareJson['aline_two_cherry']) ? $compareJson['aline_two_cherry'] : 0;
            $resultSum[$key]['三樱桃']   = isset($compareJson['aline_three_cherry']) ? $compareJson['aline_three_cherry'] : 0;
            $resultSum[$key]['橘子']    = isset($compareJson['aline_three_orange']) ? $compareJson['aline_three_orange'] : 0;
            $resultSum[$key]['芒果']    = isset($compareJson['aline_three_mango']) ? $compareJson['aline_three_mango'] : 0;
            $resultSum[$key]['西瓜']    = isset($compareJson['aline_three_watermelon']) ? $compareJson['aline_three_watermelon'] : 0;
            $resultSum[$key]['铃铛']    = isset($compareJson['aline_three_bell']) ? $compareJson['aline_three_bell'] : 0;
            $resultSum[$key]['红BAR']  = isset($compareJson['aline_three_red_bar']) ? $compareJson['aline_three_red_bar'] : 0;
            $resultSum[$key]['黄BAR']  = isset($compareJson['aline_three_yellow_bar']) ? $compareJson['aline_three_yellow_bar'] : 0;
            $resultSum[$key]['蓝BAR']  = isset($compareJson['aline_three_blue_bar']) ? $compareJson['aline_three_blue_bar'] : 0;
            $resultSum[$key]['三个七线']  = isset($compareJson['aline_three_seven']) ? $compareJson['aline_three_seven'] : 0;
            $resultSum[$key]['任意BAR'] = isset($compareJson['aline_three_any_bar']) ? $compareJson['aline_three_any_bar'] : 0;
        }
        Tool::issetInitValue($resultSum[0], array());
        return $resultSum[0];
    }
    ...
    public function LocusToPrizeMerge($locus, $compareObjs, &$result)
    {
        ...
        Tool::issetInitValue($result['win_diamond'], 0);//总送钻
        Tool::issetInitValue($result['bonus_score'], 0);//总彩金
        Tool::issetInitValue($result['prize_append_json'], 0);//记录除奖配置表以外的所有的奖
        ...
        $result['win_diamond'] += $locus['win_diamond'];
        ...
        $result['prize_append_json'] = $this->mergePrizeJson($locus['prize_append_json'], $result['prize_append_json']);
    }
    ...
    public function LocusToPrizeDay($locus, $param)
    {
        ...
        foreach ($locus as $key => $val) {
            ...
            $win_diamond     = $val['win_diamond'];//总送钻
            $bonus_score     = $val['bonus_score'];//总彩金

            $compareJson                = json_decode($val['prize_append_json'], true);//附加奖
            $compareJson['win_diamond'] = $win_diamond;//总送钻
            $compareJson['bonus_score'] = $bonus_score;//总彩金
            for ($i = 1; $i <= 5; $i++) {
                $diamondTimes               = Yii::$app->params['diamondTimes'][$i];
                $diamondCount               = Yii::$app->params['diamondCount'][$i];
            ...
            unset($val['account_id'], $val['machine_auto_id'], $val['room_index']
                , $val['play_score'], $val['win_score'], $val['play_number'], $val['win_number'], $val['win_diamond']
                , $val['bonus_score'], $val['prize_append_json']);
    ...
    public function prizeObjToChinese(&$result, $obj, $newPrizeList)
    {
        ...
        Tool::issetInitValue($result['总送钻'], 0);
        Tool::issetInitValue($result['总彩金'], 0);
        Tool::issetInitValue($result['红包来袭'], 0);

        $result['总送钻'] += $compareJsonArr['win_diamond'];//总送钻
        $result['总彩金'] += $compareJsonArr['bonus_score'];//总彩金
        //红包来袭统计
        for ($i = 1; $i <= 5; $i++) {
            $diamondTimes = Yii::$app->params['diamondTimes'][$i];
            $diamondCount = Yii::$app->params['diamondCount'][$i];
