...
class DzbPrizeDay extends Dzb
{
    ...
    public function dataCount($results)
    {
        foreach ($results as $key => $result) {
            ...
            $results[$key]['比倍后机率']  = $result['比倍总玩分数'] == 0 ? "0%" : round(($result['总赢分数'] + $result['比倍总赢分数']) / ($result['总玩分数'] + $result['比倍总玩分数']) * 100, 2) . "%";
            $results[$key]['盈利']     = round((($result['总玩分数'] + $result['比倍总玩分数']) - ($result['比倍总赢分数'] + $result['总赢分数'])) / 100, 2);
