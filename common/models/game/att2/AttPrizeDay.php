...
class AttPrizeDay extends Att
{
    ...
    public function dataCount($results)
    {
        foreach ($results as $key => $result) {
            ...
            $results[$key]['比倍后机率']  = $result['比倍总玩分数'] == 0 ? "0%" : round(($result['总赢分数'] + $result['比倍总赢分数'] + $result['彩金分数']) / ($result['总玩分数'] + $result['比倍总玩分数']) * 100, 2) . "%";
            $results[$key]['盈利']     = round((($result['总玩分数'] + $result['比倍总玩分数']) - ($result['比倍总赢分数'] + $result['总赢分数'] + $result['彩金分数'])) / 100, 2);
            ...
            $results[$key]['比倍游戏机率'] = $result['比倍总玩分数'] == 0 ? "0%" : round(($result['比倍总赢分数']) / $result['比倍总玩分数'] * 100, 2) . "%";
    ...
    public function LocusToPrize($param)
    {
        ...
        foreach ($locusObjs as $locus) {
            ...
            $this->LocusToPrizeMerge($locus, $compareObjs, $machinePrize[$locus['machine_auto_id']]);
        }
    ...
    public function LocusToPrizeMerge($locus, $compareObjs, &$result)
    {
        ...
        Tool::issetInitValue($result['bonusScore'], 0);//彩金分数
        Tool::issetInitValue($result['bonusScoreNumber'], 0);//彩金次数
        ...
        $result['bonusScore'] += $locus['bonus_score'];//为了和以前匹配所以这里的字段修改了
        if ($locus['bonus_score'] > 0) {
            $result['bonusScoreNumber'] += 1;
        }
    ...
    public function LocusToPrizeDay($locus, $param)
    {
        ...
        foreach ($locus as $key => $val) {
            ...
            $compareJson['bonusScore']           = $val['bonusScore'];//彩金分数
            $compareJson['bonusScoreNumber']     = $val['bonusScoreNumber'];//彩金次数
            ...
            unset($val['account_id'], $val['machine_auto_id'], $val['room_index']
                , $val['play_score'], $val['win_score'], $val['play_number'], $val['win_number'], $val['bonusScore'], $val['bonusScoreNumber']
    ...
    public function prizeObjToChinese(&$result, $obj, $newPrizeList)
    {
        ...
        Tool::issetInitValue($result['彩金次数'], 0);
        ...
        $result['彩金分数']   += $compareJsonArr['bonusScore'];//彩金分数
        $result['彩金次数']   += $compareJsonArr['bonusScoreNumber'];//彩金次数
