...
use common\models\game\paman\PamLocus;
use Yii;

class PamController extends GameBaseController
{
    public function __construct($gameName)
    {
        $this->gameName           = $gameName;
        $this->LocusRedisKey      = "locusPam";
        $this->LocusMaxIdRedisKey = "locusPamMaxId";
    ...
    public function Locus($time)
    {
        ...
        foreach ($newArr as $key => $val) {
            ...
            $val['id']          = $startId;
            $val['win']         -= $val['bonus_score'];
            $val['win_score']   -= $val['bonus_score'];
